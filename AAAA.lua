...
runcode(function()
	local ultrafunnytheb = {["Enabled"] = false}
	ultrafunnytheb = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
		["Name"] = "VapeIcon",
		["HoverText"] = "Gives you the vape icon beside your name on the leaderboard",
		["Function"] = function(callback)
			if callback then
                        local playerlist = game:GetService("CoreGui"):FindFirstChild("PlayerList")
                    local playerlistplayers = playerlist.PlayerListMaster.OffsetFrame.PlayerScrollList.SizeOffsetFrame.ScrollingFrameContainer.ScrollingFrameClippingFrame.ScollingFrame.OffsetUndoFrame
                    ...
                    if targetedplr then 
                        ...
                        createwarning("Vape Custom", "Disabled next round!", 2)
			end
		end 
	})
end)
		...
		["Function"] = function(callback)
			if callback then
				...
				spawn(function()
					repeat
                                        task.wait(0.3)
                                                createwarning("PlayerTp", targetedplayer and "Use This With AnitChetDisabler "..(targetedplayer.DisplayName or targetedplayer.Name) or "...1", 3)
						createwarning("PlayerTp", targetedplayer and "Teleporting "..(targetedplayer.DisplayName or targetedplayer.Name) or "Player not found", 4)
					until (not KillPlayer["Enabled"])
				end)
				spawn(function()
					repeat
